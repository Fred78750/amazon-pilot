...
**Version :** V0.10 — 03 juin 2026 (soir)
...
| **v3.6.9** | YoY Étape 4 — Free/Pro + export Word + narrative IA enrichie + Analyse par famille | 🟢 **3.6.9.2 sur staging + preprod** (03 juin soir). Validation visuelle Fred bout-en-bout en cours. Patch **v3.6.9.3 logging coûts IA** demandé par Fred avant merge prod. | **Scope figé (acté V0.9) :** (1) Toggle Vue Free/Pro **UI-only** — pas de backend Stripe (= v3.6.11/v3.8). (2) Narrative IA **semi-IA** sign='negative' uniquement via Sonnet 4 + cache IDB. (3) Section **"Analyse par famille — actions recommandées"** tableau Pro. (4) **Export Word** côté client via DOCX OOXML self-contained (3 itérations sur commit d — CDN bloqué CSP → RTF → DOCX final, cf. pattern E1). (5) **3 CTA** : 13 (Word), 14 (Toggle), 15 (Bandeaux Pro). **Logging coûts IA** : règle 35 postérieure au brief initial → **patch v3.6.9.3** demandé (store IDB `ai_usage_log` v6 + helpers debug). Mini mapping scope-livré dans `AUDIT_v3.6.9.md`. **Commits livrés** : a `cfcdd56` + b `d3e106e` + c `6ed8f26` (+ correctifs `c72e912`) + d `46acf24` → `fc07b34` → `1571541` + e `56163e3`. |
