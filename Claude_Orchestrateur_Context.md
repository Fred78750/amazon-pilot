...
|---|---|
| **Prod (main)** | **v3.7.11** — fix bug import + UX graphe (commits v3.7.9 upsert snapshot / v3.7.10 toggle Commandé-Expédié sur graphe / v3.7.11 bandeau semaines incomplètes). Historique du cycle : v3.6.9.3 → v3.6.9.4 → REFACTO bloc 1 v3.7.1→v3.7.5 (core.js −51%) → v3.7.6.1+v3.7.6.2 (perf : render ×20, save −48%) → v3.7.7 (foViews) → v3.7.8 (deliveryDefects) → **v3.7.9 fix bug import snapshot push-only → v3.7.10 → v3.7.11**. |
| **Preprod** | Alignée prod, mode **api-key directe** (pas Cognito). Un environnement **preprod Lambda pur** existe désormais (compte test Cognito) pour valider les chemins serveur — à réutiliser pour les chantiers Lambda futurs. Restaurer le mode api-key après usage. |
| **Staging / recette (CI)** | Alignés. Validation fonctionnelle bout-en-bout = **preprod uniquement** (Chrome automation impossible sur recette). |

**Cycle de CODE v3.7 CLOS.** Suite = v3.8 en phase CONCEPTION (maquettes + méthodo V3 + tag suspect défini) AVANT code. v3.7.9 fondu dans v3.8.
