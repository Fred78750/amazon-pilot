...
|---|---|
| **Prod (main)** | **v3.7.8** — bloc 2 ingestion (commit 78d24c8, tag v3.7.8). Historique du cycle : v3.6.9.3 (795035a, Free/Pro + narrative IA + logging coûts) → v3.6.9.4 (fix titres Gers) → REFACTO bloc 1 v3.7.1→v3.7.5 (core.js −51%, 5368 L) → v3.7.6.1+v3.7.6.2 (audit perf : render ×20, save −48%) → v3.7.7 (parser Traffic + timeline foViews) → v3.7.8 (parser Delivery natif + timeline deliveryDefects). Zéro régression sur tout le cycle. |
...
| **v3.7** | Refacto (bloc 1) + ingestion Traffic/Delivery (bloc 2) | ✅ **Cycle de code CLOS** (prod) | core.js −51% (5 sprints) ; audit perf (render ×20, save −48%, leçon §9 extrapolations) ; intercycle fix T1 saveUsage + preprod Lambda pur ; v3.7.7 foViews + v3.7.8 deliveryDefects. ~~v3.7.9~~ design-dépendant → **fondu dans v3.8**. |
