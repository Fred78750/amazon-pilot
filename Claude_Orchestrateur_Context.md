...
|---|---|
| **Prod (main)** | **v3.6.8.8** — mergée 29 mai 2026. **Inclut YoY Étape 3a Enquête ASINs disparus complète (parser POItemExport natif FR+EN, refonte fiche client multi-comptes BdC/Catalogue, section Marques 6 colonnes avec normalisation + alias + recalcul post-fusion, section Anomalies avec Levenshtein ≥80%, fiche détail enquête par ASIN option α expandable, 6 CTA livrés 1/2/3/6/7/8, architecture goFilteredAsins pool YoY, pattern retour α+γ).** Anticipations v3.6.9 livrées en bonus (Plan d'action P1/P2/P3, Top mouvements ASIN, Mon diagnostic narrative en dur, Conclusion générale). Tag git `v3.6.8.8`. |
...
| **v3.6.9** | YoY Étape 4 — Free/Pro + export Word + narrative IA enrichie + P4+P5 | À cadrer — scope **réduit** grâce aux anticipations v3.6.8 | Toggle Vue Free/Vue Pro + paywall back-end. Export Word automatisé. Narrative IA enrichie (vs version en dur livrée en v3.6.8). Analyse par famille Priorité 4 + Priorité 5. **NE PAS REDOUBLER** les anticipations v3.6.8 (P1/P2/P3, Top mouvements, Mon diagnostic, Conclusion). Voir AUDIT_v3.6.8.md section anticipations. ~2-3 sem (réduit vs ~3 sem initial). |
...
| **v3.7** | Refacto archi modulaire | Post-commercialisation | 3 décisions tranchées : (1) `python build.py` conservé, (2) JS vanilla maintenu, (3) découpage progressif par domaine. Slot réservé. **Inclura audit factorisation global** (règle 32). |
