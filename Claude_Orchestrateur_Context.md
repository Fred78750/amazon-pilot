...
- **V0.12 (15 juin 2026) — Phase CONCEPTION v3.8 entamée + bug bloquant prod résolu. (1) BUG IMPORT « semaine absente du graphe » diagnostiqué et corrigé (3 livraisons prod v3.7.9 / v3.7.10 / v3.7.11). Cause racine (validée empiriquement sur vrai `mergeImportData` + vraies données S22, 3 ordres de fichiers) : `import_export.js:375` créait le snapshot hebdo dès le PREMIER fichier traité (Trafic, `revenue=0`), puis le garde `alreadyArchived` (push-only) BLOQUAIT la correction par les fichiers Ventes/Stock suivants → KPIs corrects (lisent `ex.revenue`) mais graphe à 0 (lit `h.revenue` du snapshot figé) → barre invisible + N-1 idem. **Amplificateur** : passage d'Amazon au format SPLITTÉ (5 fichiers/semaine au lieu de 3) a multiplié les imports séquentiels fichier-par-fichier (chemins `autoImportFiles` core.js:1859 + `s3_poll.js:120` ; chemins batch `handleBannerCSV`/`confirmImport` non affectés). **Fix** : upsert du snapshot (`{...existing, ...snapshot}`) au lieu de push-only — chaque fichier enrichit le snapshot existant. Le spread est le BON choix (validé) : il laisse un vrai 0 mesuré écraser l'ancienne valeur → préserve le signal `passage-à-0` du tag suspect (un merge « non-null » l'aurait masqué). v3.7.10 = graphe respecte le toggle Commandé/Expédié + libellés « JJ/MM au JJ/MM ». v3.7.11 = bandeau « semaines incomplètes » (détection auto fichier manquant). **LEÇON RÉUTILISABLE (pattern « Trafic seul / snapshot push-only ») : un snapshot agrégé construit au fil d'imports séquentiels doit être upserté, jamais push-only ; et tester un fix de merge sur PLUSIEURS ORDRES de fichiers (le piège « dernier fichier gagne »).** (2) FORMAT SPLITTÉ Fabrication/Approvisionnement acté : Ventes_Fabrication = CA basé sur les COMMANDES (Commandé) / Ventes_Approvisionnement = CA basé sur les EXPÉDITIONS (Expédié) ; Stock idem (2 vues, 54 vs 48 col). `detectVCFileType` les discrimine déjà (ventes_fab/ventes_approv/stock_fab/stock_approv). Backlog : sémantique de `shippedRevenue` final = valeur Appro vs Fab (à trancher, métier). (3) IMPORT PERSONNALISÉ : les rapports hebdo Amazon ne sont disponibles que ~1,5 mois ; au-delà, rattrapage via mode « Personnaliser » (`Intervalle des rapports=[Personnaliser]`). Géré : `detectPeriodType` retourne 'weekly' pour 6-7 jours, weekKey dérivée du Champ de vision. Règle gravée : semaine ABSENTE ≠ `views:0` (clé absente = pas de donnée ; un trou interrompt la séquence « FO=0 consécutif » et le delta glissant le saute). (4) COLLISION DE NUMÉRO corrigée : « v3.7.9 » = désormais le FIX BUG IMPORT (bloquant prioritaire), PAS l'ex-tag suspect. **Le tag suspect reste en v3.8.** (5) MÉTHODO GPT V3 déposée comme référence officielle (`references/buybox/v3/` : 2 docx + README) ; V2 (md + Excel) ARCHIVÉ sous `references/buybox/archive/` (Excel V2 jugé obsolète par Fred — ne couvre ni A→P ni les 6 portes ; un modèle Excel V3 intérimaire optionnel reste possible si audit manuel avant l'Agent BB). (6) SPEC TAG SUSPECT figée en v2 (`SPEC_tag_suspect_buybox_v2.md`, staging) : signal = **dynamique des Featured Offer Page Views** (niveau, passage à 0, deltaPrevPct, deltaYoyPct, tendance), PAS un ratio. Raison : clarification GPT (12/06) « FO Page Views ≈ Glance Views pour un 1P » → un ratio FO/Glance ≈ 1, non discriminant. **Prérequis Glance Views LEVÉ** — toutes les données sont déjà dans les fichiers Traffic (FO views + 2 variations natives). À répercuter : faire reformuler par GPT le « taux d'exposition = FO/Glance » du doc V3 (incohérent avec sa propre clarification). Maille = MENSUELLE (cumul 4 imports hebdo glissants) → valide la calibration mensuelle et tue la volatilité hebdo. Double composante : détection 0 en maille hebdo (réactive) + chute/hausse en maille mensuelle glissante. Seuils calibrés sur distribution réelle Cogex FR mai (T_CHUTE −50 % = 20 % des actifs, −30 % surveiller, T_HAUSSE +50 %, FENETRE 8 sem, PLANCHER ACTIVITÉ critique car 44 % à 0 vue) — **provisoires/paramétrables**, recalibrage hebdo + Gers différé (post-accumulation). Historique foViews réel : preprod 5-6 sem, prod 9-10 sem (delta mensuel glissant déjà calculable en prod). (7) MAQUETTES UX (règle 36, workflow Orchestrateur CDC → Claude Design) : dashboard suspects v1 (Orchestrateur) + Agent_BB_Recovery + Buy_Box_Suspects + Processus_Vendor_1P (Claude Design), fidèles au système de design partagé (tokens hex, Space Grotesk/Inter/JetBrains, trace d'exposition signature, double usage défensif=ambre-rouge/offensif=teal). [À VALIDER] CDC : score qualitatif « faisceau » (≠ % arbitraire), multi-marché de la trace, étapes pipeline. Capture page Amazon = COLLAGE MANUEL (pas de scraping), réutilise l'Agent SEO. (8) LEÇONS DE MÉTHODE : (a) flags initiaux erronés assumés DEUX fois sur le bug import (intervalle « Personnaliser » puis format ligne 0) — éliminés par les faits avant de cadrer l'audit ; vérifier les FAITS (format réel des fichiers) avant d'imputer une cause ; (b) ne pas appliquer un repli proposé mécaniquement (le merge « non-null » aurait cassé le signal passage-à-0 — Claude Code a eu raison de le challenger par le test) ; (c) un audit de CODE revient à Claude Code (accès repo/IDB) — l'Orchestrateur cadre, analyse les données/symptômes, ne devine pas le code.**
- **V0.11 (12 juin 2026) — Clôture du cycle de CODE v3.7. Apports majeurs : (1) v3.6.9 + v3.6.9.3 livrées prod 03 juin (commit 795035a, tag v3.6.9.3) : toggle Free/Pro UI-only, narrative IA semi-IA, analyse par famille Pro, export Word DOCX OOXML, 3 CTA, store IDB ai_usage_log v6 + helpers. (2) v3.6.9.4 fix titres non-FR Gers (priorité FR parser_vc, XML autoritatif, re-enrichissement immédiat) ; cas B0088010BE non corrigeable (typo O/0 source Amazon). (3) REFACTO BLOC 1 COMPLET v3.7.1→v3.7.5 : core.js 10962→5368 L (−51%), méthode incrémentale par domaine validée (v3.7.1 utils+idb, v3.7.2 parsers_internal +suppression LEGACY, v3.7.3 ai_core +param optionnel inputHash, v3.7.4 import_export+s3_poll, v3.7.5 render_shell+render_screens+charts), déplacement strict zéro régression, validation Chrome automation preprod systématique. Candidats résiduels backlog : wizard/appros/fiche_amazon. (4) INTERCYCLE COGNITO + fix T1 : double verdict table ap-usage-prod vide = (b) prod tourne en mode api-key directe (Lambda jamais appelée 2xx, CloudWatch max 208ms/30j) + (a-variante) saveUsage écrivait dans ap-data-prod au lieu d'ap-usage-prod (TABLES.usage manquant dans dynamo.js). Fix T1 déployé, chaîne validée avec inputHash. Environnement preprod Lambda pur créé (compte test Cognito custom:clientId renseigné) = prérequis chantiers Lambda futurs. (5) AUDIT PERF v3.7.6 (doctrine mesurer avant optimiser, profiling Gers) : 4 goulots G1 double appel calcBuyBoxAlerts / G2 O(n²·³) reduce / G3 save full-serialize / G4 render synchrone. Fix C1=R1+R2 (render 1955→98ms ×20, sorties STRICTEMENT identiques) + fix R4 (save 611→318ms, gel synchrone JSON.stringify supprimé). R3 dirty-flag backlog pré-50k, R5 classé. LEÇON §9 GRAVÉE : extrapolation 2 points (n^2.34) = borne d'alerte PAS prédiction — Fred ne constate AUCUN gel sur Gers prod réel (le filtre getRevenue>0 réduit le N effectif <<12046). Mesurer sur le vrai dataset prod avant de dimensionner l'urgence. (6) BLOC 2 INGESTION : v3.7.7 parser Retail Analytics Traffic + timeline foViews[marché][semaine] (Featured Offer Page Views) ; v3.7.8 parser Delivery natif + timeline deliveryDefects par PO→ASIN (fin du croisement manuel). Coexistence a.glanceViews (scalaire FR courant, backward-compat) / a.foViews (timeline additive idempotente). TOUS marchés stockés (14 observés Gers, filtrage=affichage v3.8). Idempotence par clé (foViews semaine ; defects PO+weekKey+subDefect — PAS de reset total, car fichiers Delivery = fenêtres 12 mois glissantes). RÉCONCILIATION TIMELINES : foViews=début semaine (lundi), deliveryDefects=Week_end (samedi), lundi+5j=samedi. (7) FICHIER TRAFFIC HEBDO = SOURCE DE DONNÉES RÉCURRENTE (téléchargé chaque semaine depuis ~1 mois — ancien ANGLE MORT de l'Orchestrateur). 2 variantes : mono-pays (sans colonne marché, marché lu en ligne 0 Pays=[]) / multi-pays (colonne "Code de la boutique" par ligne). Métadonnées ligne 0 clé=[valeur], BOM UTF-8, nombres FR, métrique "Vues de la page de l'offre vedette". views=0 stocké comme 0 (signal central, pas null). (8) PRINCIPE DOUBLE USAGE FO views GRAVÉ : chute = défensif (BB-10/BB-11 perte d'exposition) ; hausse = offensif (réveil marché = signal d'INVESTIR un produit). Le modèle/scoring détecte les DEUX sens. (9) MÉTHODOLOGIE GPT ACTUALISÉE V3 (juin 2026, 2 docx déposés references/buybox/v3) : BB-1→12 reste la grille de classification ("quoi traiter"), COMPLÉTÉE par 16 familles de sources A→P ("pourquoi la chaîne casse") + 6 portes de triage séquentiel (1.Retail présent ? 2.buyable ? 3.compétitif ? 4.rentable ? 5.fait confiance au stock ? 6.éligible ?). Remplace livrable_audit_buybox_vendor1p_v2.md (V2 archivé). DIAGNOSTIC B00PVPXVBE ÉVOLUÉ : plus "BB-10 seul" mais consolidé BB-8 cost price non profitable/CRaP (cause dominante probable) + BB-10 + BB-3 + BB-5 + BB-4 — cas réel mixte, ne pas figer une cause trop tôt (Amazon a répondu "cost price not profitable"). (10) SIGNAL TAG SUSPECT défini = ratio taux d'exposition = Featured Offer Page Views / Glance Views (PAS la valeur FO absolue). Les 2 métriques déjà ingérées → ratio calculable en v3.8. (11) ROADMAP RECALÉE : cycle de CODE v3.7 CLOS (bloc 1 refacto + bloc 2 ingestion en prod). v3.7.9 (tag suspect + capture page + cache cross-ASIN) N'EST PAS un prérequis — design-dépendant → FONDU dans v3.8 (comme v3.6.10 fondu dans v3.7). v3.8 = phase CONCEPTION d'abord (maquettes validées + méthodo V3 déposée + tag suspect défini + [À VALIDER] CDC tranchés + retour terrain B00PVPXVBE) AVANT tout code. (12) MAQUETTES UX (règle 36) : dashboard suspects v1 produit par Orchestrateur (signature = trace d'exposition FO views, double usage défensif/offensif rendu). NOUVEAU WORKFLOW : Orchestrateur produit un CDC précis (tokens CSS + specs + limites négatives) → Claude Design exécute la maquette aboutie. CDC Agent BB Recovery produit (système de design partagé avec le dashboard, score QUALITATIF "faisceau fort/modéré/à confirmer + compteur signaux" et NON un % arbitraire, variantes défensive/offensive, limites : pas de scraping / pas d'envoi). Claude Design a produit Agent_BB_Recovery + Buy_Box_Suspects + Processus_Vendor_1P, fidèles au système de design et à la méthodo V3. (13) CAPTURE PAGE AMAZON = copier-coller MANUEL (pas de scraping : bloqué techniquement + CGU), parsing réutilise l'Agent SEO existant. (14) Patterns/leçons session : (a) présence d'une fonction en scope ≠ exécution réussie du flux — re-tester le chemin RÉSEAU réel après extraction (pas seulement le chemin cache) ; (b) idempotence ADDITIVE par clé pour fenêtres glissantes, jamais de reset total ; (c) sanity check terrain Fred contre les extrapolations (pas de gel vécu malgré ~16s extrapolés) ; (d) FLAG INITIAL ERRONÉ ASSUMÉ : avoir soupçonné Claude Design d'inventer le cadre A→P alors qu'il venait de GPT (source autoritaire commandée par Fred) — vérifier la SOURCE avant d'imputer une dérive.**
...
- **V0.10 (03 juin 2026, soir) — Apports majeurs : (1) Status report Claude Code v3.6.9 reçu et analysé : 5 commits a→e livrés sur staging (3.6.9.2 actuelle), corrections C1/C2/C3 appliquées avec preuves, arbitrages F1/F2/F3 appliqués, mini mapping scope-livré présent dans AUDIT_v3.6.9.md. (2) Étapes restantes avant merge prod v3.6.9 : validation visuelle Fred bout-en-bout sur preprod (Cogex Free + Pro, Word, narrative IA), test fonctionnel CORS Lambda ai/complete confirmé Fred (narrative IA génère bien en prod), arbitrage règle 35 = patch v3.6.9.3 décidé. (3) Patch v3.6.9.3 commandé : ajout store IDB ai_usage_log v6 (14 champs : timestamp ISO, model, tokens, coûts USD, client_id, feature, scope, success/error, duration_ms) + constante AI_PRICING_USD_PER_MILLION_TOKENS + helpers window.getAiUsageLog()/getAiUsageStats() + logging non bloquant. Pas d'UI Configuration > Usage IA (différé v3.6.10), pas de DynamoDB (différé v3.8 back admin). Instructions Claude Code dédiées produites (INSTRUCTIONS_CLAUDE_CODE_v3_6_9_3_logging_IA.md). (4) 2 nouveaux patterns d'erreur identifiés session 03 juin soir : pattern E1 (lib JS externe CDN non validée vs CSP avant brief — découverte tardive de blocage docx CDN sur commit d Export Word avec 3 itérations CDN → RTF → DOCX OOXML self-contained), pattern E2 (oubli dans liste de dépôt fichiers — INSTRUCTIONS_CLAUDE_CODE_repo_dépôt_03062026.md du matin n'incluait pas CLAUDE_CODE_v3_6_9.md ni AUDIT_v3.6.9.md, déposés ensuite naturellement par Claude Code dans merge v3.6.9 → main). (5) Règle 37 ajoutée : validation CSP/CORS des dépendances externes avant brief Claude Code (anti-pattern E1).**
...
| 37 | **Validation CSP / CORS des dépendances externes avant brief Claude Code.** Toute lib JS externe (CDN), tout endpoint API externe (Lambda, tiers), tout asset chargé dynamiquement doit être vérifié contre la Content Security Policy (CSP) de l'app ET contre les règles CORS du Lambda concerné AVANT que le brief Claude Code soit produit. **Cas concret v3.6.9 / commit d Export Word** : la lib `docx` chargée depuis CDN (jsdelivr/unpkg) a été bloquée par la CSP de l'app — pas anticipé dans le brief — résultat : 3 itérations Claude Code (commit `46acf24` CDN docx → `fc07b34` fallback RTF → `1571541` DOCX OOXML self-contained avec bibliothèque embarquée). Coût : ~6h de dev (3 itérations vs 1). **Cas concret v3.6.9 / Lambda ai/complete** : CORS partiellement résolu — `Access-Control-Allow-Origin` absent des headers visibles côté JS, status 401 reçu mais sans certitude que le bout-en-bout fonctionne. Validation différée à test fonctionnel Fred sur preprod. **Remède** : section "Dépendances externes et CSP/CORS" obligatoire dans tout brief Claude Code touchant à des libs externes, endpoints externes, ou contenus dynamiques. L'Orchestrateur doit explicitement statuer : "lib X via CDN ne passera PAS la CSP, prévoir embed self-contained" OU "lib X validée OK via CDN sur prod". | Itérations Claude Code multiples sur un même commit, perte de temps significative, brief sous-spécifié |
| 38 | **Partir du processus, pas de l'écran.** Toute maquette/refonte d'écran appartenant à un parcours multi-étapes se valide sur sa NARRATION dans le parcours (« raconte-t-elle le processus ? »), pas sur l'affichage des données. Cadre global d'abord, écran isolé ensuite. | UI muette (« rien n'a changé »), écran déconnecté du workflow, refonte coûteuse en aval |
| 39 | **Voie propre de maintenance prod.** Aucune opération de données en prod via console Fred (règle n°1). Toute maintenance prod passe par une voie technique (Claude Code / CloudShell / Chrome auto) + pipeline. « Domaine bloqué » n'autorise pas à transformer Fred en console. | Violation règle n°1, risque d'erreur manuelle en prod, aucune traçabilité |
| 40 | **Rigueur de sourcing réflexive.** Avant d'affirmer un écart/anomalie, vérifier la SOURCE de l'observation (maquette vs preprod vs prod). La règle « vérifier les faits avant d'imputer » s'applique à l'Orchestrateur lui-même. | Fausses alertes, audits inutiles, perte de crédibilité Orchestrateur |
...
- **Pattern E1 — Lib JS externe CDN non validée vs CSP avant brief Claude Code** : produire un brief incluant une dépendance JS externe (CDN) sans vérifier la compatibilité avec la Content Security Policy de l'app. Cas v3.6.9 commit d Export Word : lib `docx` via CDN jsdelivr/unpkg bloquée par CSP — pas anticipé dans le brief — résultat 3 itérations Claude Code (CDN → RTF → DOCX OOXML self-contained), coût ~6h de dev vs 1 itération si CSP vérifiée en amont. **Remède** : règle 37 ajoutée (validation CSP/CORS avant brief). Avant tout brief avec dépendance externe, statuer explicitement dans le brief : "lib X via CDN ne passera PAS la CSP, prévoir embed self-contained" OU "lib X validée OK via CDN sur prod". (Identifié 03/06/2026 — règle 37 gravée)
- **Pattern E2 — Oubli dans liste de dépôt fichiers (instructions Claude Code)** : produire des instructions de dépôt Claude Code qui n'incluent pas tous les fichiers qui devraient l'être. Cas 03/06/2026 matin : `INSTRUCTIONS_CLAUDE_CODE_repo_dépôt_03062026.md` listait 6 fichiers mais oubliait `CLAUDE_CODE_v3_6_9.md` (brief) et `AUDIT_v3.6.9.md` (mini mapping anti-Zélé). Heureusement Claude Code les inclut naturellement dans le merge v3.6.9 → main (workflow standard). **Remède** : checklist pré-production des instructions Claude Code : pour chaque version v3.x.y impactée, vérifier que (a) le brief Claude_Code_vX.Y.Z.md, (b) l'AUDIT_vX.Y.Z.md, (c) tous les nouveaux fichiers source sont bien listés dans les instructions de dépôt. (Identifié 03/06/2026 soir)
...
|---|---|
| **Prod (main)** | **v3.7.13** — hotfix modèles IA v3.7.12 + fix parser `erp_stock` v3.7.13. **Cycle v3.7 clos.** Historique du cycle : v3.6.9.3 → v3.6.9.4 → REFACTO bloc 1 v3.7.1→v3.7.5 (core.js −51%) → v3.7.6.1+v3.7.6.2 (perf : render ×20, save −48%) → v3.7.7 (foViews) → v3.7.8 (deliveryDefects) → v3.7.9→v3.7.11 fix bug import → **v3.7.12 migration modèles IA → v3.7.13 fix ERP**. |
| **Preprod** | **v3.8.3** — moteur tag suspect (v3.8.0/v3.8.1) + UI Détecter (v3.8.3), validés, NON mergés prod. Merge prod groupé avec Agent BB Recovery (décision : « à minima l'agent »). Mode **api-key directe** (pas Cognito). |
| **Staging / recette (CI)** | Alignés preprod. Validation fonctionnelle bout-en-bout = **preprod uniquement** (Chrome automation impossible sur recette). |

**Cycle de CODE v3.7 CLOS.** v3.8 phase CONCEPTION → EN COURS DE LIVRAISON. Reste : Agent BB Recovery (niveau 2), intégration page Processus, Agent Communication (v3.9).
...
| **v3.6.9 / v3.6.9.3** | YoY Étape 4 — Free/Pro + export Word + narrative IA + Analyse par famille + logging coûts IA | ✅ **Livrée prod** 03 juin (commit 795035a, tag v3.6.9.3) | Toggle Free/Pro UI-only, narrative IA semi-IA sign='negative', analyse par famille Pro, export Word DOCX OOXML self-contained, 3 CTA, store IDB `ai_usage_log` v6 + helpers. Suivi de v3.6.9.4 (fix titres non-FR Gers). |
| | **── Commercialisation été 2026 (en préparation) ──** | | Stripe test disponible. Titres Amazon ≤75 chars : audit fait (4108 NC, 69,8% CA exposé), STAND-BY observation, re-décision mi-août. |
| **v3.7** | Refacto (bloc 1) + ingestion Traffic/Delivery (bloc 2) + correctifs import | ✅ **Cycle de code CLOS** (prod v3.7.11) | core.js −51% (5 sprints) ; audit perf (render ×20, save −48%, leçon §9) ; intercycle fix T1 + preprod Lambda pur ; v3.7.7 foViews + v3.7.8 deliveryDefects ; **v3.7.9 fix bug import snapshot push-only (upsert) + v3.7.10 graphe toggle Commandé/Expédié + v3.7.11 bandeau semaines incomplètes**. ⚠️ « v3.7.9 » a été consommé par le fix bug — **le tag suspect n'est PAS v3.7.9, il est en v3.8**. Dette : workflow `deploy-preprod.yml` (pointait v3.6.7) + divergence branches main↔staging (docs only). |
| **v3.8** | **UX Buy Box refondue + Agent BB Diagnostic + (ex-v3.7.9 : tag suspect, capture page, cache cross-ASIN)** | **Phase CONCEPTION** (maquettes + méthodo V3 + tag suspect défini AVANT tout code) | **Méthodo de référence = V3 GPT (juin 2026)** : BB-1→12 (classification "quoi traiter") + 16 familles A→P (causes "pourquoi") + 6 portes de triage. **Tag suspect = ratio Featured Offer Page Views / Glance Views** (pas la valeur absolue). **Maquettes (règle 36), workflow Orchestrateur CDC → Claude Design** : dashboard suspects v1 (signature = trace d'exposition, double usage défensif/offensif) + Agent_BB_Recovery + Buy_Box_Suspects + Processus_Vendor_1P. **[À VALIDER] CDC** : score qualitatif "faisceau" (≠ % arbitraire), multi-marché de la trace, étapes pipeline. **Capture page = COLLAGE MANUEL** (pas de scraping, réutilise Agent SEO). Diagnostic pilote B00PVPXVBE = mixte BB-8+BB-10+BB-3+BB-5+BB-4. **Approche officielle Buy Box = cadre BB-1 à BB-12 V2 GPT-5** (cf. `livrable_audit_buybox_vendor1p_v2.md` + `modele_audit_buybox_vendor1p_v2.xlsx`, validés empiriquement 3 cas Cogex). **UX 3 niveaux (acté V0.9)** : (1) Visualisation ASINs suspects sur Tableau de bord et Revue Hebdo, (2) Flag "suspect" + lien Agent BB depuis Analyse ASINs, (3) **Refonte écran Buy Box** : header de synthèse conservé + liste ASINs suspects + CTA "Lancer Agent BB" remplaçant l'ancien "Enquêter". **Agent BB Diagnostic** : pattern UX type Agent SEO existant, demande inputs manquants au KAM, garde mémoire pour réutilisation cross-ASIN, applique cadre BB-1 à BB-12, restitue hypothèses restantes scorées avec **algorithme de priorisation des cas les plus probables**. **Nav existante Amazon Pilot conservée** (pas de refonte globale). **4 maquettes obligatoires** à produire AVANT brief (règle 36) : (a) landing page Buy Box, (b) fiche ASIN Recovery Agent (inspirée preview__1_.html GPT validée 03/06/2026), (c) workflow conversationnel Agent BB Diagnostic, (d) workflow Agent Communication Amazon (pour v3.9). |
