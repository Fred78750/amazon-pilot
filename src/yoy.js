...
// v3.6.5.7 — C1-C7 : Corrections signe diagnostic, KPI EUR en grand, hypothèse causale, note méthodo, tableau 4 colonnes, CSS
...
function renderYoYResult() {
  ...
  // ── KPI 4 hypothèse causale (C3 v3.6.5.7) : cause, pas symptôme
  let kpi4Icon, kpi4Label;
  if (deltaCAPct == null || Math.abs(deltaCAPct) < 3) {
    kpi4Icon = '➡️'; kpi4Label = 'Pas de signal fort';
  } else if (deltaCAPct < 0) {
    if (_disparusPctkpi > 25)                          { kpi4Icon = '📦'; kpi4Label = 'Abandon Amazon (PO non renouvelées)'; }
    else if (_disparusPctkpi > 10)                     { kpi4Icon = '📦'; kpi4Label = 'Périmètre réduit (PO raréfiées)'; }
    else if (_deltaUkpi < -10 && Math.abs(_deltaPMVkpi) < 5) { kpi4Icon = '📉'; kpi4Label = 'Ruptures structurelles'; }
    else if (_deltaPMVkpi < -5)                        { kpi4Icon = '💰'; kpi4Label = 'Pression prix / concurrence'; }
    else                                               { kpi4Icon = '⚠️'; kpi4Label = 'Causes multiples à investiguer'; }
  } else {
    if (_deltaUkpi > 10)                               { kpi4Icon = '📈'; kpi4Label = 'Hausse demande organique'; }
    else if (_deltaPMVkpi > 5)                         { kpi4Icon = '✨'; kpi4Label = 'Mix produit favorable'; }
    else                                               { kpi4Icon = '📈'; kpi4Label = 'Accélération ventes'; }
  }
  const kpi4 = `<span style="font-size:12px">${kpi4Icon}&nbsp;${esc(kpi4Label)}</span>`;
  ...
  return `<div style="max-width:960px;margin:0 auto;padding:24px 20px" class="yoy-result-root">
    ...
    <div class="yoy-kpi-grid" style="margin-bottom:24px">
      <div class="yoy-kpi-card">
        <div class="yoy-kpi-label">Évolution du CA</div>
        <div class="yoy-kpi-value ${kpi1Class}" style="font-size:20px">${deltaCAAnnu}</div>
        <div class="yoy-kpi-sub">${deltaCAPctFmt} / an</div>
      </div>
      <div class="yoy-kpi-card">
      ...
      <div class="yoy-kpi-card">
        <div class="yoy-kpi-label">Hypothèse causale</div>
        <div class="yoy-kpi-value" style="font-size:12px;line-height:1.3">${kpi4}</div>
        <div class="yoy-kpi-sub">heuristique (dims 1–7)</div>
      </div>
    </div>
