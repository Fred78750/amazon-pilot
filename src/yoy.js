...
function renderYoYResult() {
  ...
  const _cta6Html = _brandsDeclining.length > 0
    ? (function() {
        ...
        return `<div style="margin-top:10px"><button class="btn btn-sm" onclick="goToAsinsYoY(${JSON.stringify(chutingIds)}, ${JSON.stringify(label)})">Explorer les marques en chute dans Analyse ASINs →</button></div>`;
      })()
    : '';
  ...
  // v3.6.8 — CTA 7 : "Fusionner" par ligne + "Ouvrir un cas de fusion catalogue"
  const _clientRef = cl();
  const s6Rows = anomPairs.slice(0, 10).map(p => {
    ...
    // Bouton Fusionner : crée un alias canonical=marque1, variants=[marque2] avec confirmation
    const m1j = JSON.stringify(p.marque1||''), m2j = JSON.stringify(p.marque2||'');
    const fusionBtn = `<button class="btn btn-xs" onclick="yoyFusionnerMarques(${m1j},${m2j})" title="Créer un alias : ${esc(p.marque2||'')} → ${esc(p.marque1||'')}">Fusionner</button>`;
    // CTA 7 : Ouvrir cas Vendor Central (renderCaseModal si disponible)
    const casBtn = `<button class="btn btn-xs" onclick="typeof renderCaseModal==='function'?renderCaseModal(null,'fusion_catalogue',{m1:${m1j},m2:${m2j}}):go('buybox')" title="Ouvrir un cas de fusion catalogue dans Vendor Central">Cas VC →</button>`;
    return `<tr>
