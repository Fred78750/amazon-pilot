...
(function() {
  ...
  function _ooTable(headers, rows) {
    ...
    function cell(txt, isHdr) {
      return '<w:tc><w:tcPr><w:tcW w:w="' + W + '" w:type="dxa"/>'
        + '<w:shd w:val="clear" w:color="auto" w:fill="' + (isHdr ? 'E8ECF0' : 'FFFFFF') + '"/></w:tcPr>'
        + '<w:p><w:pPr><w:spacing w:after="40"/></w:pPr>'
        + _ooRun(String(txt != null ? txt : ''), isHdr ? { bold: true, size: 18 } : { size: 18 })
        + '</w:p></w:tc>';
    }
    function row(cells, isHdr) {
      return '<w:tr>' + cells.map(function(c) { return cell(c, isHdr); }).join('') + '</w:tr>';
    }
    return '<w:tbl>'
  ...
  function _buildDocumentXml(c, analysis) {
    ...
    // ── 1. Page de garde ──
    body += _ooPar('Analyse comparée — ' + (c.name || ''), { style:'Heading1', bold:true, size:32 });
    ...
    // ── 2. KPI hero ──
    body += _ooPar('1 — Évolution du chiffre d\'affaires', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 3. Catalogue ──
    body += _ooPar('2 — Dynamique catalogue', { style:'Heading2', bold:true, size:26 });
    ...
    if (d9.concRef && d9.concA) {
      body += _ooPar('3 — Concentration du portefeuille', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 5. Marques ──
    body += _ooPar('4 — Dynamique marques (Top 10)', { style:'Heading2', bold:true, size:26 });
    ...
    if (anomPairs.length > 0) {
      body += _ooPar('5 — Doublons orthographiques', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 7. Top mouvements ──
    body += _ooPar('6 — Top mouvements ASIN', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 8. Enquête ──
    body += _ooPar('7 — Enquête ASINs disparus', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 9. Plan d'action ──
    body += _ooPar('8 — Plan d\'action priorisé', { style:'Heading2', bold:true, size:26 });
    if (sign === 'negative') {
    ...
    body += _ooPar('9 — Analyse par famille — actions recommandées', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 11. Mon diagnostic ──
    body += _ooPar('10 — Mon diagnostic', { style:'Heading2', bold:true, size:26 });
    ...
    // ── 12. Conclusion ──
    body += _ooPar('11 — Conclusion générale', { style:'Heading2', bold:true, size:26 });
