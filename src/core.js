...
function renderOnboarding() {
  ...
  if (wizStep === 0) {
    ...
    const poDiscontinued = c.poData ? Object.values(c.poData).filter(d => d.isDiscontinued).length : 0;
...
function renderSEOSection(a, c) {
  ...
  if (res.backendKW) {
    ...
    h += '<div style="padding:8px 12px;background:var(--s2);border:1px solid var(--bd);border-radius:var(--rd);font-size:11px;color:var(--tx2);line-height:1.6">' + esc(res.backendKW) + '</div>';
