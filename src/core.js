...
function parseCSVFile(text, filename) {
  ...
  // Détecter la vue distributeur : Fabrication (manufacturing) vs Approvisionnement (sourcing)
  const apercu = (meta['apercu du distributeur.'] || meta['apercu du distributeur']
    || meta['distributor view'] || meta['distributor view.'] || '').toLowerCase();
  const distributorView = (apercu.includes('approvisionnement') || apercu.includes('sourcing')) ? 'appro' : 'fab';
  log(`Metadata: Pays=${meta.pays || '?'}, ${meta.entreprises || ''}, Vue=${distributorView}`);
  let periodStart = null, periodEnd = null;

  // 1. Essayer d'abord le nom du fichier — source la plus fiable
  // Format Amazon : Datedefin_DD-MM-YYYY_DD-MM-YYYY
  if (filename) {
    const fn = filename.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    const fdm = fn.match(/[Dd]atedefin[_\s]+(\d{2}-\d{2}-\d{4})[_\s]+(\d{2}-\d{2}-\d{4})/i);
    if (fdm) {
      periodStart = fdm[1].replace(/-/g, '/');
      periodEnd   = fdm[2].replace(/-/g, '/');
      log('Dates extraites du filename (Datedefin) : ' + periodStart + ' → ' + periodEnd, 'ok');
    } else {
      // Fallback : pattern DD-MM-YYYY_DD-MM-YYYY sans préfixe (ex: Chaqueannee_01-01-2024_31-12-2024)
      const fdm2 = fn.match(/(\d{2}-\d{2}-\d{4})[_\s]+(\d{2}-\d{2}-\d{4})/);
      if (fdm2) {
        periodStart = fdm2[1].replace(/-/g, '/');
        periodEnd   = fdm2[2].replace(/-/g, '/');
        log('Dates extraites du filename (brut) : ' + periodStart + ' → ' + periodEnd, 'ok');
      }
    }
  }

  // 2. Fallback sur les métadonnées du CSV si pas de dates dans le filename
  if (!periodEnd) {
  ...
  return { type: fileType, market, distributorView, periodStart, periodEnd, periodType, company: meta.entreprises || '', rowCount: data.length, data, filename };
}
...
function parseChallengeResponse(text) {
  ...
  return {
    verdictTitre:   get('VERDICT_TITRE'),
    fusionTitre:    get('FUSION_TITRE'),
    verdictB1:      get('VERDICT_B1'),
    fusionB1:       get('FUSION_B1'),
    verdictB2:      get('VERDICT_B2'),
    fusionB2:       get('FUSION_B2'),
    verdictB3:      get('VERDICT_B3'),
    fusionB3:       get('FUSION_B3'),
    verdictB4:      get('VERDICT_B4'),
    fusionB4:       get('FUSION_B4'),
    verdictB5:      get('VERDICT_B5'),
    fusionB5:       get('FUSION_B5'),
    verdictDesc:    get('VERDICT_DESC'),
    fusionDesc:     get('FUSION_DESC'),
    verdictBackend: get('VERDICT_BACKEND'),
    fusionBackend:  get('FUSION_BACKEND'),
    autocritique:   get('AUTOCRITIQUE_CLAUDE'),
    scoreClaude:    get('SCORE_CLAUDE'),
    scoreGPT:       get('SCORE_GPT'),
    titreFusion:    get('FUSION_TITRE'),
    bulletsFusion:  [
      get('FUSION_B1'), get('FUSION_B2'), get('FUSION_B3'),
      get('FUSION_B4'), get('FUSION_B5')
    ],
    verdict: [
      'Titre: ' + get('VERDICT_TITRE'),
      'B1: ' + get('VERDICT_B1'),
      'B2: ' + get('VERDICT_B2'),
      'B3: ' + get('VERDICT_B3'),
      'B4: ' + get('VERDICT_B4'),
      'B5: ' + get('VERDICT_B5'),
      'Description: ' + get('VERDICT_DESC'),
      'Backend: ' + get('VERDICT_BACKEND'),
    ].join('\n'),
  };
}
...
function renderImport() {
  ...
  const lastPPMDate = ppmLoaded ? Object.values(c.ppmData).sort((a,b)=>(b.importedAt||'').localeCompare(a.importedAt||''))[0]?.importedAt : null;
  const lastPPMStr  = lastPPMDate ? new Date(lastPPMDate).toLocaleDateString('fr-FR') : null;
  ...
  const lastFCDate = fcLoaded ? Object.values(c.forecastData).sort((a,b)=>(b.importedAt||'').localeCompare(a.importedAt||''))[0]?.importedAt : null;
  const lastFCStr  = lastFCDate ? new Date(lastFCDate).toLocaleDateString('fr-FR') : null;
...
function getEnrichedFreshness(c) {
  ...
  // ── POs : logique hebdo (semaine précédente) ────────────────────
  const lastPO = (c.pos||[]).slice().sort((a,b)=>(b.importedAt||'').localeCompare(a.importedAt||''))[0];
  const posDays = lastPO ? Math.floor((nowMs - new Date(lastPO.importedAt)) / 86400000) : null;
  const posWB   = lastPO ? weeksBehindFromDate(lastPO.importedAt) : null;
  ...
  const fcDays   = fcDates.length ? Math.floor((nowMs - new Date(fcDates[0])) / 86400000) : null;
  const fcWB     = fcDates.length ? weeksBehindFromDate(fcDates[0]) : null;
  // Prévisions OK si weeksBehind <= 2 (tolérance bimensuelle)
  function fcStatus(wb) {
    if (wb === null) return 'missing';
    if (wb <= 0) return 'ok';
    if (wb <= 2) return 'ok';   // bimensuel : 2 semaines OK
  ...
  const ppmDays  = ppmDates.length ? Math.floor((nowMs - new Date(ppmDates[0])) / 86400000) : null;
  ...
  const ytdImport = (c.imports||[])
    .filter(i => i.periodType === 'ytd' || i.type === 'ventes' && i.periodType === 'ytd')
    .sort((a,b) => new Date(b.date) - new Date(a.date))[0];
  ...
  return {
    ...
    ventesAppro: { days: lastVentesAppro ? Math.floor((nowMs-new Date(lastVentesAppro.date))/86400000) : null,
                 status: hasFabBrands ? weekStatus(vaWB) : 'ok', weeksBehind: vaWB, freq: 7, targetWeek: targetWeekHebdo, optional: !hasFabBrands },
    ...
    stockAppro:  { days: lastStockAppro ? Math.floor((nowMs-new Date(lastStockAppro.date))/86400000) : null,
                 status: hasFabBrands ? weekStatus(saWB) : 'ok', weeksBehind: saWB, freq: 7, targetWeek: targetWeekHebdo, optional: !hasFabBrands },
    pos:       { days: posDays,  weeksBehind: posWB,  status: weekStatus(posWB),      freq: 7,  targetWeek: targetWeekHebdo },
    previsions:{ days: fcDays,   weeksBehind: fcWB,   status: fcStatus(fcWB),          freq: 14, targetWeek: targetWeekHebdo },
...
function renderAgent() {
  ...
  const reportDefs = [
    ...
    { key:'pos',        icon:'📋', label:'Bons de commande', freq:'Hebdo' },
  ...
  reportDefs.forEach(function(d) {
    ...
    } else if (r.weeksBehind != null && r.weeksBehind > 0 && d.key !== 'ppm') {
...
function processImport() {
  const c = cl();
  if (!c) return;
  const files = [pendingFiles.ventes, pendingFiles.ventesAppro, pendingFiles.trafic, pendingFiles.stock, pendingFiles.stockAppro].filter(Boolean);
  ...
  setTimeout(() => smokeTest(false), 1500); // smoke test auto
}
