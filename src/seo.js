...
function buildVCModifyPrompt(asin, market, fiche, c, sku, vc) {
  ...
  var lines = [
    ...
    '    // Supprimer les emplacements bullet vides après remplissage',
    '    await sleep(500);',
    '    var allBulletFields = document.querySelectorAll(\'kat-textarea[name^="bullet_point"]\');',
    '    allBulletFields.forEach(function(field) {',
    '      var inner = field.shadowRoot && field.shadowRoot.querySelector(\'textarea\');',
    '      var val = inner ? inner.value : (field.value || \'\');',
    '      if (!val || val.trim() === \'\') {',
    '        var parent = field.closest(\'[class*="row"], [class*="item"], li, div\');',
    '        if (parent) {',
    '          var deleteBtn = parent.querySelector(\'kat-button[label*="upprimer"], button[aria-label*="upprimer"], kat-button[label*="elete"]\');',
    '          if (deleteBtn) {',
    '            var btn = deleteBtn.shadowRoot ? deleteBtn.shadowRoot.querySelector(\'button\') : deleteBtn;',
    '            if (btn) { btn.click(); console.log("[VC] Bullet vide supprimé"); }',
    '          }',
    '        }',
    '      }',
    '    });',
    ...
    '    await sleep(500);',
    '    var emballageField = document.querySelector(\'kat-dropdown[name*="emballage"], kat-dropdown[label*="mballage"], kat-dropdown[label*="ackaging"]\');',
    '    if (!emballageField) {',
    '      console.log("[VC] Champ emballage non trouvé — vérifier le sélecteur");',
    '    } else {',
    '      console.log("[VC] Champ emballage trouvé:", emballageField.getAttribute(\'name\'));',
    '      var embVal = emballageField.getAttribute(\'value\') || emballageField.value || \'\';',
    '      if (!embVal) {',
    '        emballageField.setAttribute(\'value\', \'Unité\');',
    '        emballageField.dispatchEvent(new Event(\'change\', { bubbles: true }));',
    '        await sleep(600);',
    '        var orderable = document.querySelector(\'input[type="radio"][value="true"], kat-radio-button[value="true"]\');',
    '        if (orderable) orderable.click();',
    '        await sleep(400);',
    '        console.log("[VC] Emballage → Unité + commandable = Oui");',
    '      }',
    '    }',
    '    setTimeout(function() {',
