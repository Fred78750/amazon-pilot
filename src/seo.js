...
function buildVCModifyPrompt(asin, market, fiche, c, sku, vc) {
  ...
  var lines = [
    ...
    '    console.log("OK:", name, "→", (val||"").substring(0,30));',
    '  }',
    '  fillField("item_name-0-value", ' + JSON.stringify(titre) + ');',
    '  fillField("bullet_point-0-value", ' + JSON.stringify(bullets[0] || '') + ');',
    '  fillField("bullet_point-1-value", ' + JSON.stringify(bullets[1] || '') + ');',
    '  fillField("bullet_point-2-value", ' + JSON.stringify(bullets[2] || '') + ');',
    '  fillField("bullet_point-3-value", ' + JSON.stringify(bullets[3] || '') + ');',
    '  fillField("bullet_point-4-value", ' + JSON.stringify(bullets[4] || '') + ');',
    '  fillField("rtip_product_description-0-value", ' + JSON.stringify(description) + ');',
    '  fillField("generic_keyword-0-value", ' + JSON.stringify(backendKW) + ');',
    '  var errors = [];',
