...
function buildVCModifyPrompt(asin, market, fiche, c, sku, vc) {
  ...
  var lines = [
    ...
    '    // Supprimer les emplacements bullet vides — clic répété sur "Supprimer le dernier"',
    '    await sleep(500);',
    '    for (var attempt = 0; attempt < 8; attempt++) {',
    '      var bulletFields = document.querySelectorAll(\'textarea[name^="bullet_point"], kat-textarea[name^="bullet_point"]\');',
    '      var hasEmpty = false;',
    '      bulletFields.forEach(function(f) {',
    '        var inner = f.shadowRoot && f.shadowRoot.querySelector(\'textarea\');',
    '        var val = (inner ? inner.value : f.value || \'\').trim();',
    '        if (!val) hasEmpty = true;',
    '      });',
    '      if (!hasEmpty) break;',
    '      var deleteBtn = Array.from(document.querySelectorAll(\'a, button, kat-button\'))',
    '        .find(function(el) {',
    '          return el.textContent && el.textContent.trim().match(/supprimer le dernier/i);',
    '        });',
    '      if (deleteBtn) {',
    '        deleteBtn.click();',
    '        console.log("[VC] Clic Supprimer le dernier — attempt", attempt + 1);',
    '        await sleep(600);',
    '      } else { break; }',
    '    }',
    ...
    '    // Champ Emballage — sélection via dropdown Katal',
    '    await sleep(500);',
    '    var emballageFound = false;',
    '    var allDropdowns = Array.from(document.querySelectorAll(\'kat-dropdown, select\'));',
    '    for (var ddi = 0; ddi < allDropdowns.length; ddi++) {',
    '      var dd = allDropdowns[ddi];',
    '      var ddLabel = (dd.getAttribute(\'label\') || dd.getAttribute(\'name\') || \'\').toLowerCase();',
    '      if (ddLabel.includes(\'emballage\') || ddLabel.includes(\'package\') || ddLabel.includes(\'packaging\')) {',
    '        emballageFound = true;',
    '        var currentVal = dd.getAttribute(\'value\') || dd.value || \'\';',
    '        if (!currentVal || currentVal.trim() === \'\' || currentVal.toLowerCase() === \'unit\') {',
    '          dd.click();',
    '          await sleep(400);',
    '          var options = document.querySelectorAll(\'kat-option, option\');',
    '          var uniteOpt = Array.from(options).find(function(o) {',
    '            return o.textContent && o.textContent.trim().match(/^unit[eé]$/i);',
    '          });',
    '          if (uniteOpt) {',
    '            uniteOpt.click();',
    '            console.log("[VC] Emballage → Unité sélectionné");',
    '            await sleep(400);',
    '          }',
    '          await sleep(300);',
    '          var radios = document.querySelectorAll(\'kat-radio-button, input[type="radio"]\');',
    '          radios.forEach(function(r) {',
    '            var v = r.getAttribute(\'value\') || r.value || \'\';',
    '            var lbl = (r.getAttribute(\'label\') || r.getAttribute(\'aria-label\') || (r.closest(\'label\') ? r.closest(\'label\').textContent : \'\') || \'\').toLowerCase();',
    '            if ((v === \'true\' || v === \'yes\' || lbl.includes(\'oui\')) && lbl.includes(\'command\')) {',
    '              r.click();',
    '              console.log("[VC] Commandable → Oui");',
    '            }',
    '          });',
    '        }',
    '      }',
    '    }',
    '    if (!emballageFound) console.log("[VC] Champ emballage non trouvé — vérifier sélecteur");',
    '    setTimeout(function() {',
