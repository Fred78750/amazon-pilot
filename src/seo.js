...
function buildVCModifyPrompt(asin, market, fiche, c, sku, vc) {
  ...
  var lines = [
    ...
    '    if (!el) { console.warn("NON TROUVÉ:", sel); return; }',
    ...
    '    console.log("OK:", sel);',
    '  }',
    '  fillField("textarea[name=\'item_name-0-value\']", ' + JSON.stringify(titre) + ');',
    '  fillField("textarea[name=\'bullet_point-0-value\']", ' + JSON.stringify(bullets[0] || '') + ');',
    '  fillField("textarea[name=\'bullet_point-1-value\']", ' + JSON.stringify(bullets[1] || '') + ');',
    '  fillField("textarea[name=\'bullet_point-2-value\']", ' + JSON.stringify(bullets[2] || '') + ');',
    '  fillField("textarea[name=\'bullet_point-3-value\']", ' + JSON.stringify(bullets[3] || '') + ');',
    '  fillField("textarea[name=\'bullet_point-4-value\']", ' + JSON.stringify(bullets[4] || '') + ');',
    '  fillField("textarea[name=\'rtip_product_description-0-value\']", ' + JSON.stringify(description) + ');',
    '  fillField("input[name=\'generic_keyword-0-value\']", ' + JSON.stringify(backendKW) + ');',
    '  var errors = [];',
    ...
    '  } else {',
    '    console.log("✅ Tous les champs remplis — aucune erreur détectée.");',
    '  }',
    '  console.log("ATTENDS LE GO DE FRED AVANT DE SAUVEGARDER.");',
    '})();',
    '',
    '2c. Lis les résultats console. Affiche-les moi.',
    'Attends mon GO avant de cliquer "Enregistrer et terminer".',
    '',
    '2d. Après GO : clique "Enregistrer et terminer".',
    'Confirme succès ou erreur. Passe au vendor code suivant.',
    '',
    '== ÉTAPE 3 — BILAN ==',
