...
function buildVCModifyPrompt(asin, market, fiche, c, sku) {
  ...
  var lines = [
    'CONTRAINTE TECHNIQUE CRITIQUE — LIS AVANT TOUT :',
    '- N\'utilise JAMAIS de screenshot pour vérifier les champs',
    '- Utilise UNIQUEMENT document.querySelector() pour lire et vérifier',
    '- Si une erreur d\'image apparaît, ignore-la et continue',
    '- Travaille exclusivement en JS et en texte',
    '',
    ...
    '',
    'ÉTAPE 1 — Identifier les fiches dans le catalogue',
    'Va sur vendorcentral.amazon.fr.',
    'Navigue vers le Catalogue (menu principal).',
    'Dans le champ de recherche, sélectionne "ASIN" et saisis : ' + asin,
    'Lance la recherche.',
    'Tu vas trouver une ou plusieurs lignes (une par vendor code).',
    'Si tu trouves plus de 2 vendor codes : signale-le moi avec un avertissement — risque élevé de vieux listings sans offre active.',
    'Pour chaque ligne trouvée, note le Vendor Code et le SKU.',
    'Enchaîne directement sur l\'étape 2 pour chaque vendor code trouvé.',
    '',
    'ÉTAPE 2 — Pour chaque vendor code trouvé, modifier la fiche',
    'Répète les sous-étapes suivantes pour CHAQUE ligne du catalogue :',
    '',
    '  2a — Navigation',
    '  Construis l\'URL :',
    '  https://vendorcentral.amazon.fr/abis/listing/edit/product_details?sku=[SKU_LU]&asin=' + asin + '&vendorCode=[VC_LU]#product_details',
    '  Navigue vers cette URL.',
    '  Attends le chargement complet (les textareas doivent être visibles).',
    '  Si la page affiche une erreur (SKU invalide, accès refusé, page blanche) :',
    '  → STOP sur ce vendor code.',
    '  → Indique-moi : "Le vendor code [VC_LU] ne peut pas être modifié via URL directe — intervention manuelle nécessaire."',
    '  → Passe au vendor code suivant sans modifier.',
    '',
    '  2b — Remplir le titre',
    '  Sélecteur : textarea[name="item_name-0-value"]',
    '  INSTRUCTION CRITIQUE : copier VERBATIM, sans modifier :',
    '  ' + titre,
    '',
    '  2c — Remplir les bullets',
    ...
    '',
    '  2d — Remplir la description',
    '  Exécute ce JS exactement :',
    '  (function() {',
    '  var desc = ' + JSON.stringify(description) + ';',
    '  fillField(\'textarea[name="rtip_product_description-0-value"]\', desc);',
    '  })();',
    '',
    '  2e — Remplir les mots-clés',
    '  Sélecteur : input[name="generic_keyword-0-value"]',
    '  INSTRUCTION CRITIQUE : copier VERBATIM, sans modifier :',
    '  ' + backendKW,
    '',
    '  2f — Vérification erreurs + récapitulatif',
    '  Exécute ce JS pour détecter les champs obligatoires manquants :',
    '  (function() {',
    '  var errors = [];',
    ...
    '    console.log("STOP — signale ces erreurs à Fred avant de soumettre.");',
    '  } else {',
    '    console.log("✅ Aucune erreur détectée — prêt à soumettre. Attends le GO de Fred.");',
    '  }',
    '  })();',
    '  Affiche-moi le résultat de ce JS + récapitulatif des champs remplis.',
    '  Attends mon GO avant de cliquer "Enregistrer et terminer".',
    '',
    '  2g — Après validation',
    '  Clique "Enregistrer et terminer".',
    '  Confirme-moi le succès ou l\'erreur.',
    '  Passe au vendor code suivant s\'il en reste.',
    '',
    'ÉTAPE 3 — Bilan final',
    'Liste tous les vendor codes traités avec statut succès/erreur.',
    '',
